            ...
            <div class="order-md-last">
                <h4 class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    @if(isset($latestOrder) && $latestOrder)
                        <span class="badge bg-primary rounded-pill">{{ $latestOrder->orderProduks->count() }}</span>
                    @else
                ...
                <ul class="list-group mb-3">
                    @if(isset($latestOrder) && $latestOrder)
                        @foreach($latestOrder->orderProduks as $item)
                            <li class="list-group-item d-flex justify-content-between lh-sm">
                                <div>
                                    <h6 class="my-0">{{ $item->produk->nama }}</h6>
                                    <small class="text-body-secondary">{{ $item->quantity }}x</small>
                                    @if($item->produk->deskripsi)
                                        <small class="text-body-secondary d-block">
                                            {{ \Illuminate\Support\Str::limit($item->produk->deskripsi, 50) }}
                                        </small>
                                    @endif
                                </div>
